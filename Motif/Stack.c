...
#define Offset(field) XtOffsetOf(StackRec, stack.field)
...
#define Offset(field) XtOffsetOf(StackConstraintRec, stack.field)
...
static void internal_StackSetActiveChild(StackWidget self, int i)
{
    if (i < 0) i = 0;
    ...
    self->stack.last_active_child = self->stack.active_child;
    self->stack.active_child = i;
}

static int internal_StackGetActiveChild(StackWidget self)
{
    return(self->stack.active_child);
}

static void Initialize(Widget req_widget, Widget new_widget, ArgList args, Cardinal *num_args)
{
    StackWidget self = (StackWidget)new_widget;

    if (self->core.width <= 0) self->core.width = DEFAULT_WIDTH;
    if (self->core.height <= 0) self->core.height = DEFAULT_HEIGHT;

    self->stack.active_child = -1;
    self->stack.last_active_child = -1;
}

static int internal_StackChildWidgetOrder(StackWidget self, Widget child)
{
    if (self)
    {
	int i;
...
static void DisplayActiveChild(StackWidget self)
{
    Widget child, folder;

    if (XtIsRealized(self) && self->stack.last_active_child != self->stack.active_child)
    {
	if (self->stack.last_active_child >= 0)
	{
	    child = self->composite.children[self->stack.last_active_child];

	    XtCallCallbackList((Widget)self, self->stack.now_hidden_cb, child);
	}

	if (self->stack.active_child >= 0)
	{
	    child = self->composite.children[self->stack.active_child];
	    ...
	    if (folder && XtClass(folder) == folderWidgetClass) FolderRedisplayTabsNotify(folder);
	    ...
	    XtCallCallbackList((Widget)self, self->stack.now_displayed_cb, child);
	}
    }
}
...
static void Resize(Widget w)
{
    ...
    Position margin_space = self->stack.outside_margin + self->core.border_width;
...
static void Realize(Widget w, XtValueMask *value_mask, XSetWindowAttributes *attributes)
{
    ...
    stackWidgetClass->core_class.superclass->core_class.realize(w, value_mask, attributes);

    if (self->stack.active_child >= 0)
    {
	Widget child = self->composite.children[self->stack.active_child];
...
static XtGeometryResult handle_layout(StackWidget self, Widget requesting_child,
...
#define RestoreGeometry()   do { self->core.width = saved_width; self->core.height = saved_height; } while (0)
    ...
    margin_space = self->stack.outside_margin * 2 + self->core.border_width * 2;
...
static XtGeometryResult GeometryManager(Widget requesting_child,
					XtWidgetGeometry *desired, XtWidgetGeometry *allowed)
{
    return(handle_layout((StackWidget)XtParent(requesting_child), requesting_child, desired, allowed));
}
...
static void ChangeManaged(Widget w)
{
    ...
    folder = XtParent(self);
    if (folder && XtClass(folder) == folderWidgetClass) FolderTabsChangedNotify(folder);

    DisplayActiveChild(self);
}

static StackWidget StackWidgetCast(Widget w)
{
    WidgetClass widget_class = w->core.widget_class;

    while (widget_class != 0 && widget_class != coreWidgetClass)
    {
	if (widget_class == stackWidgetClass)
	{
	    return((StackWidget)w);
	}
	if (widget_class == folderWidgetClass)
	{
...
int StackNumChildren(Widget w)
{
    StackWidget self = StackWidgetCast(w);
...
int StackChildWidgetOrder(Widget w)
{
    StackWidget self = StackWidgetCast(XtParent(w));

    return(internal_StackChildWidgetOrder(self, w));
}

void StackNextWidget(Widget w)
{
    StackWidget self = StackWidgetCast(w);
...
void StackPreviousWidget(Widget w)
{
    StackWidget self = StackWidgetCast(w);
...
void StackGotoWidget(Widget w, int i)
{
    StackWidget self = StackWidgetCast(w);
...
void StackSetActiveChild(Widget w, int i)
{
    StackWidget self = StackWidgetCast(w);
...
int StackGetActiveChild(Widget w)
{
    StackWidget self = StackWidgetCast(w);
...
StackClassRec stackClassRec = 
{
	...
	/* constraint_size          */ sizeof(StackConstraintRec), 
	/* initialize               */ 0,
	/* destroy                  */ 0,
	/* set_values               */ 0,
	/* extension                */ 0
